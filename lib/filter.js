...
Filter.prototype.parse = function() {
    ...
    regex = pathToRegexp( this.pattern, null, {
        sensitive: true,
        strict: true
    });
 ...
 * @return {Object}
 */
function getFSTreeMather ( filters ) {
    var filters = formatFilter(filters);

    var inFilters = _.where( filters, { type: 'include' } ),
        exFilters = _.where( filters, { type: 'exclude' } );

    return {
        //files filters
        matches: function ( file, isDir ) {
            var matched = _.every( inFilters, function ( filter ) {
                    return filter.match( file, isDir );
                } ),
                mismatched = _.any( exFilters, function ( filter ) {
                    return filter.match( file, isDir );
                } );
        ...
        //folder filters
        excludes: function ( folder, isDir ) {
            return _.any( exFilters, function ( filter ) {
                return filter.match( folder, isDir );
            } );
        }
    };
}
...
exports.getFSTreeMather = getFSTreeMather;
